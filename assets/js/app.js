...
function visibleNodes(search){const out=[];const rec=(node,level,parent='')=>{const kids=[...node.children.values()];kids.forEach((c,idx)=>{const isProgram=!!c.item;const open=uiState.openFolders.has(c.path)||uiState.selectedPath.startsWith(c.path+' > ');const cnt=isProgram?(search.program.get(c.path)?.count||0):(search.folderCounts.get(c.path)||0);const vis=!search.q.active||search.visible.has(c.path)||cnt>0; if(!vis)return; out.push({path:c.path,type:isProgram?'program':'folder',label:c.name,level,parentPath:parent,isOpen:open,isSelected:uiState.selectedPath===c.path&&uiState.selectedType===(isProgram?'program':'folder'),isFocused:uiState.focusedPath===c.path, count:cnt,posInSet:idx+1,setSize:kids.length}); if(!isProgram&&open)rec(c,level+1,c.path);});}; rec(treeRoot,1); return out;}
