...
function highlight(t,s){const q=s.q;if(!q.active)return esc(t);const src=String(t||''),query=q.raw.toLowerCase();let out='',last=0,i;while((i=src.toLowerCase().indexOf(query,last))>=0){out+=esc(src.slice(last,i))+'<mark>'+esc(src.slice(i,i+query.length))+'</mark>';last=i+query.length;}return out+esc(src.slice(last));}
...
function renderProgram(search){const p=byPath[uiState.selectedPath];if(!p)return;const s=specs[p.path]||{},lanes=s.lanes||[{title:s.title||p.name,blocks:s.blocks||[]}];document.getElementById('program').innerHTML=`<div class="pathline">${esc(p.path)}</div><div class="program-frame" style="--pw:${s.width||420}px"><div class="viewtabs" role="tablist"><button role="tab" aria-selected="${uiState.activeView==='patient'}" data-view="patient">Patient View</button><button role="tab" aria-selected="${uiState.activeView==='basic'}" data-view="basic">Basic Patient View</button></div><div class="lanes" style="--cols:${lanes.map(l=>l.weight?`${l.weight}fr`:'1fr').join(' ')}">${lanes.map(l=>`<section class="lane"><div class="lane-head">${highlight(l.title,search)}</div><div class="flow">${(l.blocks||[]).map(b=>renderBlock(b,search)).join('')}</div></section>`).join('')}</div></div>`;applyZoom();}
const renderRow=(b,s)=>`<div class="row"><div><span>${highlight(b.name||'',s)}</span>${b.time?`<span class="rtime">${highlight(b.time,s)}</span>`:''}</div><div>${b.name?PERSON_SVG:''}${b.badge?`<span class="badge">${highlight(b.badge,s)}</span>`:''}${b.pill?`<span class="pill">${highlight(b.pill,s)}</span>`:''}</div></div>`;
