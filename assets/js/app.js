...
function renderRow(b, state) { const blank = !b.name; return `<div class="row${blank ? ' blank' : ''}" title="${esc(blank ? '' : [b.name, b.time, b.pill].filter(Boolean).join(' · '))}"><div class="row-top"><span class="rname${blank ? ' rblank' : ''}">${highlightText(b.name || '', state)}</span>${b.time ? `<span class="rtime">${highlightText(b.time, state)}</span>` : ''}</div><div class="row-bot"><div class="ricons">${blank ? '' : PERSON_SVG}${b.badge ? `<span class="badge" title="${esc(b.badge)}">${highlightText(b.badge, state)}</span>` : ''}</div>${b.pill ? `<span class="pill" title="${esc(b.pill)}"><span class="pill-text">${highlightText(b.pill, state)}</span></span>` : ''}</div></div>`; }
function renderBlock(b, state) { if (!b) return ''; if (b.t === 'spacer') return `<div class="spacer" style="--spacer-rows:${Math.max(1, Number(b.n || 1) || 1)}"></div>`; if (b.t === 'label') return `<div class="label${b.tone === 'orange' ? ' orange' : ''}">${highlightText(b.text || '', state)}</div>`; if (b.t === 'decision') { const cols = b.cols || []; const defaultText = b.default || 'Nein'; return `<div class="decision-q"><div class="dq-top"><span class="qtext">${highlightText(b.q || 'Decision', state)}</span></div><div class="dq-bot"><span class="dropdown" title="${esc(defaultText)}"><span class="dropdown-text">${highlightText(defaultText, state)}</span></span><span class="pill" title="Basic Decision"><span class="pill-text">Basic Decision</span></span></div></div><div class="decision-title" title="${esc(b.title || b.q || 'Decision')}">${highlightText(b.title || b.q || 'Decision', state)}</div><div class="branch-grid" style="--bcols:${cols.length || 1}">${cols.map((c) => `<div class="branch"><div class="branch-head" title="${esc(c.label || '')}">${highlightText(c.label || '', state)}</div>${(c.blocks || []).map((x) => renderBlock(x, state)).join('')}</div>`).join('')}</div>`; } return renderRow(b, state); }
