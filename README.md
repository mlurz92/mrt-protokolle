...
Behoben durch strukturelle (nicht nur kosmetische) Absicherung: `.dropdown` besitzt jetzt `max-width:52%` mit innerem `.dropdown-text` (Ellipsis); `.pill` ist auf `flex:0 1 auto` mit innerem `.pill-text` (Ellipsis) und einer Mindestbreite (`min-width:40px`, zeigt also immer noch ein paar Zeichen vor dem Ellipsis) umgestellt, das feste Kreis-/Dreieck-Icon bleibt via `flex-shrink:0` unverändert; `.badge` hat `max-width` + Ellipsis als Sicherheitsnetz; `.branch-head` und `.decision-title` erhielten `overflow:hidden`/`text-overflow:ellipsis`/`white-space:nowrap`. Alle trunkierten Elemente tragen ein `title`-Attribut mit dem vollständigen Text (Tooltip bei Hover). Damit ist die Kollisionsfreiheit **unabhängig von zukünftigen Datenänderungen strukturell garantiert**, nicht nur für den aktuellen Datenstand empirisch geprüft.

**Komfortabler Sicherheitsabstand statt knappem Minimum:** Ein reines "nicht überlappend" reichte nicht aus – `.row-bot`/`.dq-bot` garantieren jetzt `gap:12px` (vorher 8px) zwischen Icon/Badge-Gruppe und Pill/Dropdown, `.row-top`/`.dq-top` haben zusätzlich `gap:10px` zwischen Name und Zeit. Die Zeilenhöhe (`--row-h`) wurde von 37px auf 43px erhöht, `.row-bot`/`.dq-bot` von 19px auf 21px, `.pill`/`.dropdown` von 17–18px auf 18–19px Höhe mit etwas mehr Innenabstand (Padding) – insgesamt spürbar mehr Luft, ohne die kompakte Workstation-Dichte des Grunddesigns aufzugeben. Ein automatisierter Abstandsmessung über alle 60 echten Programme bestätigt: der knappste real vorkommende horizontale Abstand zwischen zwei Elementen liegt jetzt exakt beim garantierten Minimum von 12px (vorher konnte er nahe 0px liegen).

**Vertikaler Abstand zwischen Name/Zeit-Zeile und Icon/Pill-Zeile:** `.row` und `.decision-q` (beides `flex-direction:column`-Container aus je zwei Zeilen: `.row-top`/`.dq-top` mit Name+Zeit, `.row-bot`/`.dq-bot` mit Icon/Badge+Pill/Dropdown) hatten zuvor `gap:0` – die Sequenzname-Zeile und z. B. das „MPR Assignment"-Pill darunter berührten sich direkt ohne jeden Luftraum. Jetzt `gap:2px` auf beiden Containern (Zeilenhöhe entsprechend von 41px auf 43px erhöht, damit die einzelnen Zeilen dabei nicht schrumpfen). Per Playwright gemessen: garantierter vertikaler Mindestabstand jetzt 2px in allen 60 echten Programmen (vorher 0px).
