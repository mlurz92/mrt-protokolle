...
**Komfortabler Sicherheitsabstand statt knappem Minimum:** Ein reines "nicht überlappend" reichte nicht aus – `.row-bot`/`.dq-bot` garantieren jetzt `gap:12px` (vorher 8px) zwischen Icon/Badge-Gruppe und Pill/Dropdown, `.row-top`/`.dq-top` haben zusätzlich `gap:10px` zwischen Name und Zeit. Die Zeilenhöhe (`--row-h`) wurde von 37px auf 41px erhöht, `.row-bot`/`.dq-bot` von 19px auf 21px, `.pill`/`.dropdown` von 17–18px auf 18–19px Höhe mit etwas mehr Innenabstand (Padding) – insgesamt spürbar mehr Luft, ohne die kompakte Workstation-Dichte des Grunddesigns aufzugeben. Ein automatisierter Abstandsmessung über alle 60 echten Programme bestätigt: der knappste real vorkommende Abstand zwischen zwei Elementen liegt jetzt exakt beim garantierten Minimum von 12px (vorher konnte er nahe 0px liegen).
